...
test("path globs stop at slashes unless doubled", () => {
  ...
  assert.ok(
    globToRegExp("**/*.ts", true).test("index.ts"),
...
test("the shipped defaults never park: nobody is at Make.com's terminal", async () => {
  ...
  for (const command of ["npm test", "terraform apply", "sudo ls"]) {
    ...
    assert.notEqual(
      decision.verdict,
      "ask",
      "an unanswered ask is a 15-minute stall; the default must not ask",
    );
  }
});
...
test("the ask list: deny still wins, allow still passes, and only what every segment covers is asked", async () => {
  ...
  const exec = (command: string) =>
    decider.decide({ ...base, kind: "exec", command, cwd: "/work" }, context);
  const write = (path: string) =>
    decider.decide(
      { ...base, kind: "write", changes: [{ path, change: "update" }] },
      context,
    );
  ...
  assert.equal(
    (await exec("ls && terraform apply")).verdict,
    "ask",
    "an allowed segment plus an asked one is a question",
  );
  assert.equal(
    (await exec("terraform apply && sudo ls")).verdict,
    "deny",
    "one denied segment makes the whole line a refusal, never a question",
  );
  assert.equal(
    (await exec("echo $(cat ~/.ssh/id_rsa)")).verdict,
    "deny",
    "unjudgeable stays refused even when ask would match",
  );
  assert.equal((await write("src/a.ts")).verdict, "ask");
  assert.equal((await write(".env.local")).verdict, "deny");
  const unmatched = await decider.decide(
    { ...base, kind: "net", host: "example.com" },
    context,
  );
  assert.equal(
    unmatched.verdict,
    "deny",
    "a kind with no ask pattern is still refused",
  );
  ...
  assert.equal(
    (
      await strict.decide(
        { ...base, kind: "exec", command: "terraform apply", cwd: "/work" },
        context,
      )
    ).verdict,
    "deny",
...
test("secrets are protected from reading as well as writing", async () => {
  const decider = new BuiltinDecider(DEFAULT_CONFIG.decide);
  const read = await decider.decide(
    { ...base, kind: "read", paths: ["/work/.env"] },
    context,
  );
  assert.equal(read.verdict, "deny");
  const pem = await decider.decide(
    { ...base, kind: "read", paths: ["/work/certs/server.pem"] },
    context,
  );
  assert.equal(pem.verdict, "deny");
  const ok = await decider.decide(
    { ...base, kind: "read", paths: ["/work/src/index.ts"] },
    context,
  );
...
test("every shell operator separates segments, and what text cannot judge is refused", async () => {
  ...
  assert.equal(
    (await exec("rg -n -i 'foo|bar (baz|qux)|[0-9]+' src")).verdict,
    "allow",
  );
  assert.equal(
    (await exec("git ls-files | rg -v '(^|/)package-lock\\.json$'")).verdict,
    "allow",
  );
  assert.equal((await exec("rg -n '=>' src")).verdict, "allow");
  assert.equal(
    (await exec("echo '$HOME'")).verdict,
    "allow",
    "single quotes make $ literal",
  );
  assert.equal(
    (await exec('echo "$HOME"')).verdict,
    "deny",
  ...
  assert.equal(
    (await exec("find . -name x -exec echo {} \\;")).verdict,
    "deny",
    "find -exec runs a command of its own",
  );
  assert.equal(
    (await exec("grep 'x$' f")).verdict,
    "allow",
    "a $ before a closing quote is literal",
  );
...
test("a command that names a file is judged as a read of that file, by its real path", async () => {
  ...
  const judge = async (command: string, lists = DEFAULT_CONFIG.decide) => {
    ...
    return new BuiltinDecider(lists).decide(contained.operation, {
      ...context,
  ...
  assert.equal(
    (await judge("ls confidential", custom)).verdict,
    "allow",
    "naming the directory is not reading the files in it",
  );
});
...
test("deny rules see through wrappers, shell keywords, parentheses and the program's path", async () => {
  ...
  assert.match(
    (await decider.decide(exec("env -S 'curl http://x'"), context)).reason,
    /options to env/,
  );

  const defaults = new BuiltinDecider(DEFAULT_CONFIG.decide);
  assert.equal(
    (await defaults.decide(exec("command -v node"), context)).verdict,
    "allow",
  );
  assert.equal(
    (await defaults.decide(exec("env CI=1 npm test"), context)).verdict,
    "allow",
    "a harmless env prefix is stripped for the allow list too",
  );
  assert.equal(
    (await defaults.decide(exec("./bin/git status"), context)).verdict,
    "deny",
    "a workspace script is not git, whatever it is called",
  );
  assert.equal(
    (await defaults.decide(exec("/usr/bin/env node --version"), context)).verdict,
    "deny",
    "allow rules never match by bare program name",
  );
});
...
test("a search over a directory is judged by every file it can reach", async () => {
  ...
  const search = (dir: string): Operation => ({
    ...base,
    kind: "read",
    recursive: true,
    paths: [dir],
  ...
  assert.equal(
    (await narrow.decide(search(ws), ctx)).verdict,
    "deny",
    "a reached file the allow list does not cover refuses the search",
  );
...
test("a search that honours .gitignore does not reach what git ignores, and only git's word counts", async () => {
  ...
  assert.equal(
    ...
    "deny",
    "no repository: nothing is ignored",
  );
  ...
  assert.equal(
    (await defaults.decide(search, ctx)).verdict,
    "deny",
    "a .gitignore that does not name .env changes nothing",
  );
