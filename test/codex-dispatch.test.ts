...
function harness(scenario: Record<string, unknown>, decide: (operation: Operation) => Decision | Promise<Decision>) {
  ...
  const log = () => readFileSync(join(dataDir, "fake-log.jsonl"), "utf8").trim().split("\n").map((line) => JSON.parse(line));
...
test("every Codex approval request is answered from the gateway's decision, and a withdrawn one is left alone", async () => {
  const scenario = {
    steps: [
      { notify: "item/started", params: { item: { type: "commandExecution", id: "item_1", command: "/bin/zsh -lc 'npm test'", cwd: "/tmp" } } },
      { request: "item/commandExecution/requestApproval", params: { itemId: "item_1", command: "/bin/zsh -lc 'npm test'", cwd: "/tmp", reason: "run the tests" } },
      { notify: "item/commandExecution/outputDelta", params: { itemId: "item_1", delta: "1 passing\n" } },
      { notify: "item/completed", params: { item: { type: "commandExecution", id: "item_1", exitCode: 0 } } },
      // The file-change approval carries no diff; it arrived on item/started.
      { notify: "item/started", params: { item: { type: "fileChange", id: "item_2", changes: [{ path: "notes.md", diff: "+hello", kind: { type: "add" } }] } } },
      { request: "item/fileChange/requestApproval", params: { itemId: "item_2", reason: "write notes" } },
      { notify: "item/completed", params: { item: { type: "fileChange", id: "item_2", status: "completed", changes: [{ path: "notes.md", diff: "+hello", kind: { type: "add" } }] } } },
      { request: "item/permissions/requestApproval", params: { permissions: { network: { enabled: true } }, reason: "fetch a package" } },
      { request: "item/commandExecution/requestApproval", params: { itemId: "item_3", command: "cat ~/.ssh/id_rsa" } },
      // Withdrawn before we answer: serverRequest/resolved follows immediately.
      { request: "item/commandExecution/requestApproval", params: { itemId: "item_4", command: "sleep 100" }, withdraw: true },
      { request: "mcpServer/elicitation/request", params: { message: "pick one" } },
      { request: "item/tool/requestUserInput", params: { questions: [{ id: "q1", text: "sure?" }] } },
      { notify: "item/agentMessage/delta", params: { delta: "All done." } },
      { notify: "turn/completed", params: { turn: { id: "turn_fake", status: "completed" } } },
    ],
  };
  ...
  assert.deepEqual(
    ...
    ["exec:npm test", "write:notes.md", "net", "exec:cat ~/.ssh/id_rsa", "exec:sleep 100"],
  );
  const write = h.asked.find((op) => op.kind === "write");
  assert.equal(write?.kind === "write" ? write.changes[0]?.diff : null, "+hello", "the diff was joined onto the approval by itemId");
  ...
  const answered = h.log().filter((entry) => entry.answered).map((entry) => entry.answered);
  assert.deepEqual(
  ...
  assert.ok(!h.log().some((entry) => entry.unexpectedAnswer), "a withdrawn request is never answered");
  assert.ok(slowResolve, "the withdrawn request had reached the gateway before it was withdrawn");
  ...
  for (const expected of ["command.started", "command.output", "command.finished", "files.changed", "text"] as const) assert.ok(types.includes(expected), `emits ${expected}`);
  const started = h.events.find((event) => event.type === "command.started");
  assert.equal(started?.type === "command.started" ? started.command : null, "npm test");
...
test("a Codex that cannot list its MCP servers does not get to run", async () => {
  const h = harness({ mcpListFails: true, steps: [] }, () => ({ verdict: "allow", reason: "" }));
  await assert.rejects(h.provider.start(h.context), /could not list its MCP servers .*Refusing to run/);
  assert.ok(!h.log().some((entry) => entry.received === "turn/start"), "no turn was started");
});

test("a Codex with a foreign MCP server enabled is refused before the turn starts", async () => {
  const h = harness({ mcpServers: [{ name: "evil", tools: { shell: {} } }], steps: [] }, () => ({ verdict: "allow", reason: "" }));
...
test("cancelling a run interrupts the turn and ends it as cancelled", async () => {
  const h = harness({ steps: [{ request: "item/commandExecution/requestApproval", params: { itemId: "i", command: "sleep 5" } }] }, () => new Promise(() => {}));
  const handle = await h.provider.start(h.context);
...
test("a permission request is answered only with the entries that were judged and allowed — never echoed back", async () => {
  ...
  const scenario = {
    steps: [
      ...
      { request: "item/permissions/requestApproval", params: { permissions: { fileSystem: { entries: [{ access: "write", path: { type: "special", value: { kind: "root" } } }, { access: "read", path: { type: "path", path: `${homedir()}/.ssh` } }] } }, reason: "need it" } },
      // A plain read outside the workspace: judged, contained, refused.
      { request: "item/permissions/requestApproval", params: { permissions: { fileSystem: { entries: [{ access: "read", path: { type: "path", path: `${homedir()}/.ssh` } }] } } } },
      // A write inside the workspace plus a read outside it: only the write comes back.
      { request: "item/permissions/requestApproval", params: { permissions: { fileSystem: { entries: [{ access: "write", path: { type: "path", path: "build" } }, { access: "read", path: { type: "path", path: "/etc" } }] } } } },
      // Network: never grantable through this channel.
      { request: "item/permissions/requestApproval", params: { permissions: { network: { enabled: true } } } },
      { notify: "turn/completed", params: { turn: { id: "turn_fake", status: "completed" } } },
    ],
  };
  // Stands in for the gateway: containment refuses anything outside the workspace, the rules allow the rest.
  const h = harness(scenario, (operation) => {
    const paths = operation.kind === "read" ? operation.paths : operation.kind === "write" ? operation.changes.map((c) => c.path) : [];
    if (operation.kind === "net") return { verdict: "deny", reason: "no destination" };
    if (paths.some((p) => p.startsWith("/") && !p.startsWith(h.root))) return { verdict: "deny", reason: "outside the workspace" };
    return { verdict: "allow", reason: "rules say yes", scope: "session" };
  });
  const handle = await h.provider.start(h.context);
  await handle.done;
  const answers = h.log().filter((e) => e.answered).map((e) => e.answered.result);
  assert.deepEqual(answers[0], { permissions: {}, scope: "turn" }, "special target: refused before any rule");
  assert.deepEqual(answers[1], { permissions: {}, scope: "turn" }, "read outside the workspace: refused");
  assert.deepEqual(answers[2], { permissions: { fileSystem: { entries: [{ access: "write", path: { type: "path", path: "build" } }] } }, scope: "session" }, "only the judged, allowed entry is granted");
  assert.deepEqual(answers[3], { permissions: {}, scope: "turn" }, "network is refused");
  // The gateway was asked about every translatable path and nothing else.
  const kinds = h.asked.map((op) => (op.kind === "read" ? `read:${op.paths.join(",")}` : op.kind === "write" ? `write:${op.changes.map((c) => c.path).join(",")}` : op.kind));
  assert.deepEqual(kinds, [`read:${homedir()}/.ssh`, "read:/etc", "write:build", "net"], "every translatable path is judged; the request with a special target never reaches the gateway at all");
