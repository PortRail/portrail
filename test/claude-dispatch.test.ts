...
function fakeSdk(
  script: (captured: Captured, emit: (message: any) => void) => Promise<void>,
) {
...
test("the PreToolUse hook is the gate — every tool call is decided by the gateway and answered in the SDK's shape", async () => {
  ...
  const sdk = fakeSdk(async (captured, emit) => {
    ...
    emit({
      ...
      tool_use_result: { stdout: "1 passing\n", stderr: "" },
    });
    hookAnswers.push(
      await ask(
        "Write",
        { file_path: join(captured.options.cwd, "notes.md"), content: "hi" },
        "tu_2",
      ),
    );
    emit({
      ...
      tool_use_result: { filePath: join(captured.options.cwd, "notes.md") },
    });
  ...
  assert.deepEqual(
    hookAnswers.map(
      (answer) => answer.hookSpecificOutput?.permissionDecision ?? answer.behavior,
    ),
    ["allow", "allow", "deny", "deny", "deny", "deny"],
  );
  assert.match(
    hookAnswers[4].hookSpecificOutput.permissionDecisionReason,
    /not one Portrail can govern/,
  );
  ...
  assert.equal(
    options.sandbox.failIfUnavailable,
    true,
    "no sandbox means no run, unless the operator opts out",
  );
  assert.equal(
    typeof options.stderr,
    "function",
    "a degraded sandbox is surfaced, not swallowed",
  );
  for (const secret of [
    "**/.codex/**",
    "**/.claude/**",
    ...
    assert.ok(options.sandbox.filesystem.denyRead.includes(secret), secret);
  ...
  assert.ok(
    !("env" in options) && !("ANTHROPIC_API_KEY" in options),
    "the SDK falls back to the user's own login",
  );
...
test("a Claude that starts with a tool Portrail did not ask for is stopped at init", async () => {
  const sdk = fakeSdk(async (_captured, emit) => {
    emit(
      init({
        tools: [...GOVERNED_TOOLS, "mcp__github__create_issue"],
  ...
  assert.match(
    outcome.summary,
    /did not ask for \(mcp__github__create_issue, mcp:github\)/,
  );
  assert.ok(
...
test("a second run in the same session resumes the SDK session, and steering is refused honestly", async () => {
  ...
  const h = harness(sdk, () => ({ verdict: "allow", reason: "" }));
  const handle = await h.provider.start({
    ...h.context,
    nativeSessionId: "claude-ses-1",
  });
...
test("the operator's read denies are enforced by the sandbox as well, translated to the workspace", async () => {
  ...
  const provider = new ClaudeProvider({
    sdk: sdk as any,
    denyRead: ["confidential/**", "**/*.key", ".secrets"],
  });
  const events: ProviderEvent[] = [];
  const abort = new AbortController();
  const handle = await provider.start({
    ...
    signal: abort.signal,
    emit: (event) => events.push(event),
    decide: async () => ({ verdict: "allow", reason: "" }),
  });
  await handle.done;
  const denyRead: string[] = sdk.captured[0]!.options.sandbox.filesystem.denyRead;
  assert.ok(
    denyRead.includes(join(root, "confidential/**")),
    "a workspace-relative rule is anchored at the workspace",
  );
  assert.ok(
    denyRead.includes("**/*.key"),
    "a rule that already matches anywhere stays as it is",
  );
...
test("losing the Claude stream after the agent started leaves the outcome unknown; before it, a plain failure", async () => {
  ...
  assert.equal(
    await outcomeOf(silentAfterInit),
    "outcome_unknown",
    "a stream that ends without a result after the agent started",
  );
  ...
  assert.equal(
    await outcomeOf(lostBeforeInit),
    "failed",
    "nothing had started, so nothing is unknown",
  );
});
