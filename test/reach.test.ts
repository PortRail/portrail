...
function fixture() {
  ...
  const outside = realpathSync.native(mkdtempSync(join(tmpdir(), "portrail-reach-outside-")));
  for (const dir of ["src", "confidential", "node_modules/pkg", ".git/objects", "sub"]) mkdirSync(join(ws, dir), { recursive: true });
  for (const file of [".env", ".envrc", "src/a.ts", "confidential/plan.txt", "node_modules/pkg/server.pem", ".git/objects/x", "sub/y.txt", "README.md"])
...
const names = (files: string[], ws: string) => files.map((f) => f.slice(ws.length + 1)).sort();
...
test("a walk lists what a search can reach: hidden files only when asked, never node_modules or .git", () => {
  ...
  assert.deepEqual(names(visible.files, ws), ["README.md", "confidential/plan.txt", "src/a.ts", "sub/y.txt"]);
  ...
  assert.deepEqual(names(hidden.files, ws), [".env", ".envrc", "README.md", "confidential/plan.txt", "src/a.ts", "sub/y.txt"]);
  assert.ok(!hidden.files.some((f) => f.includes("node_modules") || f.includes("/.git/")), "dependency and history trees are skipped");
});
...
test("symlinks are followed only when the tool would follow them, and a link out of the workspace is reported", () => {
  ...
  assert.deepEqual(names(stay.files, ws), ["sub/y.txt"], "a symlink is not a file when links are not followed");
  const followed = reachableFiles(join(ws, "sub"), ws, { hidden: true, follow: true });
  assert.deepEqual(names(followed.files, ws), ["sub/y.txt"], "a directory loop terminates and lists each file once");
  const whole = reachableFiles(ws, ws, { hidden: true, follow: true });
  assert.equal(whole.outside, join(ws, "escape.txt"), "the link that leaves the workspace is named");
  const alias = reachableFiles(ws, ws, { hidden: false, follow: true });
  assert.ok(alias.outside === join(ws, "escape.txt") || alias.files.includes(join(ws, ".env")), "an alias resolves to the real file, or the walk stops at the escaping link first");
  void outside;
});
