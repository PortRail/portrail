...
test("the systemd unit quotes every token and escapes what systemd interprets", () => {
  ...
  assert.match(
    unit,
    /^ExecStart="\/usr\/bin\/node" "\/opt\/p\/bin\/portrail\.mjs" "start" "--relay" "https:\/\/x\/%%s"$/m,
  );
...
test("the launchd plist escapes XML and points its log at the chosen data directory", () => {
  ...
  assert.match(
    plist,
    /<key>StandardOutPath<\/key><string>\/tmp\/elsewhere\/portrail\.log<\/string>/,
  );
...
test("the service hints follow the chosen data directory", () => {
  assert.ok(
    serviceInfo("darwin", "/tmp/elsewhere").hints.some(
      (hint) => hint === "tail -f /tmp/elsewhere/portrail.log",
    ),
  );
});
