...
import { approve, deny, describeOperation, exitCodeFor, run, status } from "../src/cli/commands.ts";
...
async function daemonForThisProcess(decider = new BuiltinDecider({ allow: [], deny: [], ask: ["exec:*"] })) {
  ...
  const app = await createApp({ gateway: ctx.gateway, store: ctx.store, keys, dataDir: home, extension: null, localToken, agentStatus: async () => [] });
  ...
  return { ...ctx, home, app, logs, token, close: async () => { console.log = original; await app.close(); await ctx.gateway.shutdown(); } };
}
...
test("portrail run --json exits 2 when something was refused and 1 when the run did not succeed, like the text mode", async () => {
  const d = await daemonForThisProcess(new BuiltinDecider({ allow: ["exec:npm test"], deny: ["exec:rm*"], ask: [] }));
  const args = (...argv: string[]) => parseArgs([...argv, "--home", d.home]);
  try {
    const refused = await run(args("run", script([{ exec: "rm -rf build" }]), "--agent", "fake", "--key", d.token, "--json"));
    assert.equal(refused, 2, "a refused operation is exit 2 in JSON mode too");
    const failed = await run(args("run", script([{ fail: "nope" }]), "--agent", "fake", "--key", d.token, "--json"));
    assert.equal(failed, 1);
    const clean = await run(args("run", script([{ exec: "npm test" }]), "--agent", "fake", "--key", d.token, "--json"));
    assert.equal(clean, 0);
    assert.ok(d.logs.some((line) => /"state": "failed"/.test(line)), "the JSON output still carries the final run");
