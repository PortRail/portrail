...
export async function init(options: InitOptions) {
  ...
  try {
    ...
    if (options.workspace) {
      ...
      const name =
        options.workspaceName ?? root.split("/").filter(Boolean).pop() ?? "workspace";
      const existing = gateway
        .listWorkspaces()
        .find((w) => w.root === root || w.name === name);
        ...
        JSON.stringify(
          {
            ...
            created,
            workspace,
            key: key ? { ...key, token: key.token } : null,
    ...
    if (!workspaces.length)
      todo.push(
        "portrail workspace add /path/to/project --name project   (or: portrail init --workspace .)",
      );
    if (!keys.length)
      todo.push(
        "portrail key create local                                (or: portrail init --key local)",
      );
