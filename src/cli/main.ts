...
async function main(): Promise<number> {
  ...
  switch (args.command) {
    ...
    default: {
      ...
      const command = extension?.commands?.[args.command];
      if (command) {
        const { dataDirectory, ensurePrivateDirectory } = await import("../store/paths.ts");
        return command.run({ positional: args.positional, flags: args.flags, dataDir: ensurePrivateDirectory(dataDirectory(home)) });
      }
