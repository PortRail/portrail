...
export function serviceInfo(platform: NodeJS.Platform = process.platform): ServiceInfo {
  ...
  return {
    ...
    hints:
      platform === "darwin"
        ? [`launchctl print gui/$(id -u)/${LABEL}`, `tail -f ${logPath()}`]
        : ["systemctl --user status portrail", "journalctl --user -u portrail -f"],
  };
}

const logPath = () => join(process.env.PORTRAIL_HOME ?? join(homedir(), ".portrail"), "portrail.log");
...
export function installService(options: { dataDir: string; extraArgs?: string[] }): ServiceInfo {
  ...
  const cli = resourcePath("bin", "portrail.mjs");
...
${args.map((arg) => `    <string>${escapeXml(arg)}</string>`).join("\n")}
  ...
  <key>EnvironmentVariables</key><dict>
    <key>PORTRAIL_HOME</key><string>${escapeXml(options.dataDir)}</string>
...
`;
    writeFileSync(info.unitPath, plist, { mode: 0o600 });
...
Environment=PORTRAIL_HOME=${options.dataDir}
...
`;
    writeFileSync(info.unitPath, unit, { mode: 0o600 });
